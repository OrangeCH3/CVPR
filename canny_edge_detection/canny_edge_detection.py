...
for i in range(1, img2.shape[0]-1):
    for j in range(1, img2.shape[1]-1):
        if (theta[i, j] == 0.0) and \
                (img1[i, j] == np.max([img1[i, j], img1[i+1, j], img1[i-1, j]])):
            img2[i, j] = img1[i, j]

        if (theta[i, j] == -45.0) and \
                img1[i, j] == np.max([img1[i, j], img1[i-1, j-1], img1[i+1, j+1]]):
            img2[i, j] = img1[i, j]

        if (theta[i, j] == 90.0) and \
                img1[i, j] == np.max([img1[i, j], img1[i, j+1], img1[i, j-1]]):
            img2[i, j] = img1[i, j]

        if (theta[i, j] == 45.0) \
                and img1[i, j] == np.max([img1[i, j], img1[i-1, j+1], img1[i+1, j-1]]):
            img2[i, j] = img1[i, j]
